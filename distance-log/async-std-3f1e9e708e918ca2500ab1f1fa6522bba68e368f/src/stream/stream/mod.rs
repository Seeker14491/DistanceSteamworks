...
extension_trait! {
    ...
    pub trait StreamExt: futures_core::stream::Stream {
        ...
        #[doc=r#"
            Await a stream or times out after a duration of time (like [`timeout`](#method.timeout))
            but reset timeout repeatly when stream poll ready (data ready or timeout).
            ...
            let mut s = stream::repeat(1).take(3).timeout_repeat(Duration::from_secs(1));
            ...
            // when timeout
            let mut s = stream::pending::<()>().timeout_repeat(Duration::from_millis(10));
            match s.next().await {
                Some(item) => assert!(item.is_err()),
                None => panic!()
            };
        ...
        #[cfg(any(feature = "unstable", feature = "docs"))]
        #[cfg_attr(feature = "docs", doc(cfg(unstable)))]
        fn timeout_repeat(self, dur: Duration) -> Timeout<Self>
        where
            Self: Stream + Sized,
        {
            Timeout::new_repeat(self, dur)
        }
