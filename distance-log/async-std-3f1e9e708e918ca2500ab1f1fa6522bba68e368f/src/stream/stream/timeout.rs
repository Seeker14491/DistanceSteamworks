...
impl<S: Stream> Timeout<S> {
    pub(crate) fn new(stream: S, dur: Duration) -> Self {
        ...
        Self { stream, delay, duration: None }
    }

    pub(crate) fn new_repeat(stream: S, dur: Duration) -> Self {
        let delay = Delay::new(dur);

        Self { stream, delay, duration: Some(dur) }
    }
}
...
impl<S: Stream> Stream for Timeout<S> {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        if let Some(dur) = this.duration {
            this.delay.as_mut().reset(Instant::now() + *dur);
        }
